...
public final class JourneyNEIToggleWidget
    ...
    public void renderObjects(GuiContainer gui, int mousex, int mousey) {
        ...
        // Draw dropdowns last so an open menu is not painted underneath the compact header buttons.
        groupDropdown.draw(mousex, mousey);
        orderDropdown.draw(mousex, mousey);
    }
    ...
    public boolean mouseClicked(GuiContainer gui, int mousex, int mousey, int mouseButton) {
        ...
        // Open menus own their option rectangles before any item-panel click may see them.
        if (groupDropdown.isOpen() && groupDropdown.click(mousex, mousey, mouseButton)) return true;
        if (orderDropdown.isOpen() && orderDropdown.click(mousex, mousey, mouseButton)) return true;
        if (groupDropdown.click(mousex, mousey, mouseButton)) return true;
        if (orderDropdown.click(mousex, mousey, mouseButton)) return true;
        if (click(neiButton, mousex, mousey, mouseButton)) return true;
        if (click(researchButton, mousex, mousey, mouseButton)) return true;
        if (click(favouriteButton, mousex, mousey, mouseButton)) return true;
        if (click(creativeButton, mousex, mousey, mouseButton)) return true;
        if (click(deleteButton, mousex, mousey, mouseButton)) return true;
        if (click(latestButton, mousex, mousey, mouseButton)) return true;
        if (scanVisible && click(scanButton, mousex, mousey, mouseButton)) return true;
        return debugToolVisible && click(debugToolButton, mousex, mousey, mouseButton);
    }
