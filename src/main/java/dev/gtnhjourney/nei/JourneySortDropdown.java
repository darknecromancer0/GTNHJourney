...
final class JourneySortDropdown {
    ...
    void place(JourneyHeaderLayout.Slot slot) {
        place(slot, Integer.MAX_VALUE);
    }

    void place(JourneyHeaderLayout.Slot slot, int screenWidth) {
        if (slot == null) return;
        ...
        if (open) {
            for (OptionButton option : options) {
                if (option.contains(mousex, mousey)) {
                    option.handleClick(mousex, mousey, mouseButton);
                    return true;
                }
            }
    ...
    boolean containsOpenPopup(int mousex, int mousey) {
        return containsPopup(mousex, mousey);
    }

    boolean containsAny(int mousex, int mousey) {
        return mainButton.contains(mousex, mousey) || containsPopup(mousex, mousey);
    }
    ...
    private void rebuildOptions() {
        ...
        JourneyViewState.Mode view = JourneyViewState.mode();
        if (kind == Kind.GROUP) {
            for (JourneyGroupMode value : JourneyGroupMode.values()) {
                options.add(new OptionButton(value.abbreviation() + "  " + value.label(), value, null));
            }
        } else {
    ...
    private void rebuildOptionGeometry() {
        int popupX = JourneyDropdownViewport.clampPopupX(mainButton.x, OPTION_WIDTH, viewportWidth);
        int y = mainButton.y + mainButton.h + OPTION_GAP;
        for (OptionButton option : options) {
            option.x = popupX;
            option.y = y;
            option.w = OPTION_WIDTH;
            option.h = OPTION_HEIGHT;
            option.z = 300;
