...
final class JourneySortDropdown {
    ...
    private String activeAbbreviation() {
        ...
        return kind == Kind.GROUP
            ? JourneySortState.group(view).abbreviation()
            : JourneySortState.order(view).abbreviation();
    }

    private String mainTooltip() {
        JourneyViewState.Mode view = JourneyViewState.mode();
        if (kind == Kind.GROUP) {
            return "Group: " + JourneySortState.group(view).label() + ". Click to choose None/N/M/T/K.";
        }
        String extra = view == JourneyViewState.Mode.FAVOURITE ? "/F+" : "";
        return "Order: " + JourneySortState.order(view).label() + ". Click to choose None/U/A" + extra + ".";
    }
