...
public final class ResearchTrace {
    ...
    public static void unlocked(EntityPlayerMP player, ItemStack stack) {
        ...
        try {
            ...
            String text = "unlock " + key.getItemId() + " @" + key.getMeta()
                + (key.getCanonicalNbt().isEmpty() ? " [base]" : " [NBT]")
                + " GT=" + GtChargeStatePolicy.describe(stack)
                + " IC2=" + Ic2ChargeStatePolicy.describe(stack);
            ...
            FMLLog.info("[GTNH Journey trace %s] unlock identity failed: %s", player.getUniqueID(), failure.getMessage());
        }
    }
}
