...
public final class JourneyNetwork {
    ...
    public static void init() {
        ...
        CHANNEL.registerMessage(ResearchUnlockNotificationMessage.Handler.class, ResearchUnlockNotificationMessage.class, 8, Side.CLIENT);
        CHANNEL.registerMessage(ResearchActivitySyncChunkMessage.Handler.class, ResearchActivitySyncChunkMessage.class, 9, Side.CLIENT);
        CHANNEL.registerMessage(ResearchActivityTouchMessage.Handler.class, ResearchActivityTouchMessage.class, 10, Side.CLIENT);
        ...
        CHANNEL.registerMessage(ResearchIssuedSyncChunkMessage.Handler.class, ResearchIssuedSyncChunkMessage.class, 16, Side.CLIENT);
        CHANNEL.registerMessage(ResearchIssuedTouchMessage.Handler.class, ResearchIssuedTouchMessage.class, 17, Side.CLIENT);
    }

    public static void requestRetrieve(ResearchKey key, int amount) { CHANNEL.sendToServer(new RetrieveRequestMessage(ResearchFingerprint.of(key), amount)); }
    public static void requestFillInventory(ResearchKey key) { if (key != null) CHANNEL.sendToServer(new FillInventoryRequestMessage(ResearchFingerprint.of(key))); }
    ...
    public static void requestDelete(ResearchKey key) { if (key != null) CHANNEL.sendToServer(new DeleteRequestMessage(ResearchFingerprint.of(key))); }
    public static void requestInventoryScan() { CHANNEL.sendToServer(new InventoryScanRequestMessage()); }
    public static void requestDebugTool() { CHANNEL.sendToServer(new DebugToolRequestMessage()); }
    ...
    public static void sendFullSync(EntityPlayerMP player, List<ItemStack> stacks) {
        ...
        List<ResearchKey> issued = player == null ? java.util.Collections.<ResearchKey>emptyList()
            : dev.gtnhjourney.GTNHJourney.RESEARCH.snapshotIssuedOrder(player);
        ServerResearchSyncQueue.start(player, stacks, activity, issued);
    }

    public static void sendFullSync(EntityPlayerMP player, List<ItemStack> stacks, List<ResearchKey> activityOldestFirst) {
        List<ResearchKey> issued = player == null ? java.util.Collections.<ResearchKey>emptyList()
            : dev.gtnhjourney.GTNHJourney.RESEARCH.snapshotIssuedOrder(player);
        ServerResearchSyncQueue.start(player, stacks, activityOldestFirst, issued);
    }
    ...
    public static void sendActivityTouch(EntityPlayerMP player, ResearchFingerprint fingerprint) {
        ...
        if (!ServerResearchSyncQueue.deferActivityTouchIfActive(player, fingerprint)) sendActivityTouchImmediate(player, fingerprint);
    }
    ...
    public static void sendIssuedTouch(EntityPlayerMP player, ResearchFingerprint fingerprint) {
        if (player == null || fingerprint == null) return;
        if (!ServerResearchSyncQueue.deferIssuedTouchIfActive(player, fingerprint)) sendIssuedTouchImmediate(player, fingerprint);
    }
    static void sendIssuedTouchImmediate(EntityPlayerMP player, ResearchFingerprint fingerprint) {
        if (player != null && fingerprint != null) sendToConnected(new ResearchIssuedTouchMessage(fingerprint), player);
    }
    ...
    static void sendSyncBegin(EntityPlayerMP player, int epoch, int availableTotal, int syncableTotal, int activityTotal,
        int issuedTotal, boolean normalizeGtTransientIdentity, boolean resetGtToolTemplateState,
        boolean normalizeGtChargeEndpoints, boolean normalizeIc2ChargeEndpoints, boolean normalizeTconToolWear,
        boolean normalizeCofhChargeEndpoints) {
        sendToConnected(new ResearchSyncBeginMessage(epoch, availableTotal, syncableTotal, activityTotal, issuedTotal,
            normalizeGtTransientIdentity, resetGtToolTemplateState, normalizeGtChargeEndpoints,
            normalizeIc2ChargeEndpoints, normalizeTconToolWear, normalizeCofhChargeEndpoints), player);
    }

    static void sendSyncChunk(EntityPlayerMP player, int epoch, List<ItemStack> chunk) { sendToConnected(new ResearchSyncChunkMessage(epoch, chunk), player); }
    static void sendActivitySyncChunk(EntityPlayerMP player, int epoch, List<ResearchFingerprint> chunk) { sendToConnected(new ResearchActivitySyncChunkMessage(epoch, chunk), player); }
    static void sendIssuedSyncChunk(EntityPlayerMP player, int epoch, List<ResearchFingerprint> chunk) { sendToConnected(new ResearchIssuedSyncChunkMessage(epoch, chunk), player); }
    static void sendSyncEnd(EntityPlayerMP player, int epoch) { sendToConnected(new ResearchSyncEndMessage(epoch), player); }
