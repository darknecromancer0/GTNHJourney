...
/** Compact chunk of oldest-first N activity fingerprints belonging to one full research-sync epoch. */
public final class ResearchActivitySyncChunkMessage implements IMessage {

    private static final int MAX_ENTRIES = 512;
    private int epoch;
    private final List<ResearchFingerprint> fingerprints = new ArrayList<ResearchFingerprint>();

    public ResearchActivitySyncChunkMessage() {}

    public ResearchActivitySyncChunkMessage(int epoch, List<ResearchFingerprint> fingerprints) {
        this.epoch = epoch;
    ...
    public static final class Handler implements IMessageHandler<ResearchActivitySyncChunkMessage, IMessage> {

        @Override
        public IMessage onMessage(ResearchActivitySyncChunkMessage message, MessageContext ctx) {
            ...
            ClientNetworkQueue.enqueue(new Runnable() {
                ...
                @Override
                public void run() {
                    ClientActivityMirror.addChunk(epoch, copy);
                }
            });
            return null;
        }
