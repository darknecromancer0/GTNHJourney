...
public final class ItemDebugResearcherTool extends Item {
    ...
    public static InteractionDecision route(DebugResearchMode mode, boolean sneaking, boolean hasTarget) {
        ...
        if (hasTarget || selected == DebugResearchMode.AREA_16) return new InteractionDecision(InteractionAction.EXECUTE, true);
    ...
    private static void execute(final EntityPlayerMP player, ItemStack tool, final DebugResearchMode mode,
        ...
        scanner.apply(mode, x, y, z, new DebugResearchScanService.MutationAdapter() {
            @Override public void createSafetySnapshot(String name) { GTNHJourney.MUTATIONS.createSafetySnapshot(player, name); }
            @Override public int applyBulkAdd(List<ItemStack> candidates, String description) { return GTNHJourney.MUTATIONS.applyBulkAdd(player, candidates, description); }
        }, new DebugResearchScanService.ActionEffects() {
            @Override public void sync() {
                JourneyNetwork.sendFullSync(player, GTNHJourney.RESEARCH.snapshotStacksInUnlockOrder(player), GTNHJourney.RESEARCH.snapshotActivityOrder(player));
            }
            @Override public void summary(DebugResearchMode selected, DebugResearchScanResult result) {
                JourneyRuntimeCounters.debugResearchScan(
                    result.getPositionsVisited(), result.getInventoriesVisited(), result.getDroppedItemsVisited(),
                    result.getUniqueCandidates(), result.getNewlyUnlockedStates());
    ...
    static String formatSummary(DebugResearchMode mode, DebugResearchScanResult result) {
        ...
        DebugResearchScanResult safe = result == null
            ? new DebugResearchScanResult(java.util.Collections.<ItemStack>emptyList(), 0, 0, 0, 0, 0, 0)
            : result;
        ...
        switch (selected) {
            ...
            case AREA_16:
                return "Migration AREA_16: " + safe.getPositionsVisited() + " positions, " + safe.getBlockCandidates()
                    + " block candidates, " + safe.getInventoriesVisited() + " inventories, "
                    + safe.getDroppedItemsVisited() + " dropped items, +" + added + states;
            case BLOCK:
            default:
    ...
    private static int playerX(EntityPlayer player) { return MathHelper.floor_double(player.posX); }
    private static int playerY(EntityPlayer player) { return MathHelper.floor_double(player.posY); }
    private static int playerZ(EntityPlayer player) { return MathHelper.floor_double(player.posZ); }
    private static void tell(EntityPlayerMP player, String text) { player.addChatMessage(new ChatComponentText("[Journey] " + text)); }

    public enum InteractionAction { CYCLE_MODE, EXECUTE, NO_TARGET }

    public static final class InteractionDecision {
        private final InteractionAction action;
        private final boolean consumed;
        InteractionDecision(InteractionAction action, boolean consumed) { this.action = action; this.consumed = consumed; }
        public InteractionAction action() { return action; }
